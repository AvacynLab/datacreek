...
def parse_qa_pairs(text: str) -> List[Dict[str, str]]:
    ...
    if verbose:
        logging.getLogger(__name__).debug("Parsing response of length %d", len(text))
    ...
    try:
        ...
        if "[" in text and "]" in text:
            ...
            try:
                ...
                if verbose:
                    logging.getLogger(__name__).debug("Successfully parsed %d QA pairs", len(pairs))
                return pairs
        ...
        if verbose:
            logging.getLogger(__name__).debug("Error during JSON extraction: %s", e)

    # Fallback to regex pattern matching
    if verbose:
        logging.getLogger(__name__).debug("Falling back to regex pattern matching")
    ...
    for match in re.finditer(qa_pattern, text):
        try:
            ...
            if verbose:
                logging.getLogger(__name__).debug("Error extracting pair: %s", e)

    if verbose:
        if pairs:
            logging.getLogger(__name__).debug("Extracted %d QA pairs with regex", len(pairs))
        else:
...
def parse_ratings(text: str, original_items: List[Dict[str, str]] = None) -> List[Dict[str, Any]]:
    ...
    if verbose:
        ...
        logger.debug("Raw response: %s", repr(text[:500]))
    ...
    try:
        ...
        if "{" in json_content and "}" in json_content:
            ...
            try:
                ...
                if isinstance(parsed, dict) and "rating" in parsed:
                    if verbose:
                        logging.getLogger(__name__).debug("Successfully parsed single JSON object")
                    return [parsed]
            except json.JSONDecodeError as e:
                if verbose:
                    logging.getLogger(__name__).debug("JSON parse error for object: %s", str(e))
        ...
        if "[" in json_content and "]" in json_content:
            ...
            try:
                ...
                if verbose:
                    logging.getLogger(__name__).debug("JSON parse error for array: %s", str(e))

    except Exception as e:
        if verbose:
            logging.getLogger(__name__).debug("Error in primary parsing approach: %s", str(e))
    ...
    try:
        ...
        if verbose:
            logging.getLogger(__name__).debug("Error in code block extraction: %s", str(e))
    ...
    try:
        ...
        if verbose:
            logging.getLogger(__name__).debug("Error in regex extraction: %s", str(e))
    ...
    try:
        ...
        if verbose:
            logging.getLogger(__name__).debug("json5 not available")
    ...
    try:
        ...
        if verbose:
            logging.getLogger(__name__).debug("Error in final extraction attempt: %s", str(e))

    # If we reach here, we couldn't extract valid JSON
    if verbose:
        logging.getLogger(__name__).debug("All parsing methods failed")
