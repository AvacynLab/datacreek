...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    logging.getLogger(__name__).debug("Loading config from: %s", config_path)
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)

    # Debug: Print LLM provider if it exists
    if "llm" in config and "provider" in config["llm"]:
        logging.getLogger(__name__).debug("Config LLM provider: %s", config["llm"]["provider"])
    else:
        logging.getLogger(__name__).debug("Config does not have LLM provider set")
...
def get_llm_provider(config: Dict[str, Any]) -> str:
    ...
    logging.getLogger(__name__).debug("get_llm_provider returning: %s", provider)
    if (
        ...
        logging.getLogger(__name__).warning(
            "Config has 'api-endpoint' but returning '%s'", provider
        )
    return provider
