...
class LLMClient:
    ...
    def _openai_chat_completion(
        ...
        if verbose:
            logger.info("Sending request to %s model %s...", self.provider, self.model)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if verbose:
                    logger.info("Received response from %s", self.provider)
                ...
                content = self._extract_openai_content(response, verbose)
                return content
    ...
    def _extract_openai_content(self, response: Any, verbose: bool) -> str:
        """Extract the assistant content from an OpenAI response object."""
        ...
        try:
            ...
            if verbose:
                logger.info("Standard format extraction failed: %s", e)
        ...
        try:
            ...
            if verbose:
                logger.info("Llama API format extraction failed: %s", e)

        # Method 3: Generic dict access
        try:
            response_dict = None
            if hasattr(response, "model_dump"):
            ...
            if verbose:
                logger.info("Dictionary access failed: %s", e)
