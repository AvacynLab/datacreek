...
class DummyClient:
    def __init__(self):
        self.config = {"prompt": "Q", "generation": {"temperature": 0.1, "max_tokens": 5, "batch_size": 1}}
...
class DummyDataset:
    ...
    def map(self, func, batch_size=None, batched=False):
        ...
        return self
    def to_dict(self):
        out = {"image": [], "query": list(self.data["query"]), "label": list(self.data["label"])}
...
def test_process_dataset_from_file(monkeypatch):
    ...
    monkeypatch.setattr(builtins, "open", lambda *_args, **_kw: (_ for _ in ()).throw(FileNotFoundError()))
    gen = vqa.VQAGenerator(DummyClient())
...
def test_process_dataset_with_backend(monkeypatch):
    ...
    ds_mod = DummyDatasetsModule()
    img = Image.new("RGB", (1,1))
    ds_mod.load_dataset = lambda _src: DummyDataset({"image":[img],"query":["q"],"label":["a"]})
    monkeypatch.setitem(sys.modules, "datasets", ds_mod)
    class DummyAPI: repo_exists=lambda self, **_: True
    hf_mod = types.ModuleType("huggingface_hub"); hf_mod.HfApi=lambda: DummyAPI()
    monkeypatch.setitem(sys.modules, "huggingface_hub", hf_mod)
    monkeypatch.setattr(builtins, "open", lambda *_a, **_k: (_ for _ in ()).throw(FileNotFoundError()))
    backend = DummyBackend()
    gen = vqa.VQAGenerator(DummyClient())
    key = gen.process_dataset("dummy_repo", num_examples=1, input_split="train", backend=backend, redis_key="k")
    assert key == "k"
