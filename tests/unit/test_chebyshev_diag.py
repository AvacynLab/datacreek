...
def test_chebyshev_diag_fallback(monkeypatch):
    ...
    L = np.diag([1.0, 2.0])
    diag = cd.chebyshev_diag_hutchpp(L, t=0.5, order=3, samples=10, rng=np.random.default_rng(0))
...
def test_chebyshev_diag_scipy():
    ...
    L = np.diag([0.0, 2.0])
    diag = cd.chebyshev_diag_hutchpp(L, t=0.1, order=5, samples=50, rng=np.random.default_rng(1))
    expected = np.array([1.0, np.exp(-0.2)])
    assert sorted(np.round(diag, 4)) == pytest.approx(sorted(np.round(expected, 4)), rel=1e-2)
