...
def test_process_batches_success(caplog):
    ...
    with caplog.at_level(logging.ERROR):
        res = batch.process_batches(client, msgs, batch_size=1, temperature=0.0, parse_fn=parse)
...
def test_process_batches_parse_error(caplog):
    ...
    with caplog.at_level(logging.ERROR):
        out = batch.process_batches(client, [[{'a':1}]], batch_size=1, temperature=0.0, parse_fn=bad_parse)
    assert out == []
...
def test_process_batches_raise_on_error():
    ...
    with pytest.raises(RuntimeError):
        batch.process_batches(client, [[{'a':1}]], batch_size=1, temperature=0.0, parse_fn=str, raise_on_error=True)


def test_process_batches_log_error(caplog):
    client = DummyClient(raise_error=True)
    with caplog.at_level(logging.ERROR):
        res = batch.process_batches(client, [[{'a':1}]], batch_size=1, temperature=0.0, parse_fn=str)
    assert res == []
...
async def test_async_process_batches_success():
    client = DummyClient()
    res = await batch.async_process_batches(client, [[{'m':1}], [{'m':2}]], batch_size=1, temperature=0.0, parse_fn=lambda x: x)
    assert res == ['ok1', 'ok1']
