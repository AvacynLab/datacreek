...
from datacreek.models.results import CurationResult, CurationMetrics
from datacreek.models.qa import QAPair

class DummyClient:
    def __init__(self):
        self.config = {
            "curate": {"threshold": 7.0, "batch_size": 2, "inference_batch": 1, "temperature": 0.1},
    ...
    def batch_completion(self, msgs, *, temperature, batch_size):
        return asyncio.run(self.async_batch_completion(msgs, temperature=temperature, batch_size=batch_size))

def setup_dummy(monkeypatch):
    client = DummyClient()
    monkeypatch.setattr(curate, "LLMClient", lambda **_: client)
    monkeypatch.setattr(curate, "get_curate_settings", lambda cfg: types.SimpleNamespace(**cfg["curate"]))
...
async def test_curate_qa_pairs_async_invalid(monkeypatch):
    ...
    with pytest.raises(ValueError):
        await curate.curate_qa_pairs_async({"qa_pairs": [{"question":"q","answer":"a"}]}, threshold=11)
...
async def test_curate_qa_pairs_async_parse_error(monkeypatch):
    ...
    with pytest.raises(curate.CurationError):
        await curate.curate_qa_pairs_async({"qa_pairs": [{"question":"q","answer":"a"}]})


@pytest.mark.asyncio
async def test_curate_qa_pairs_async_resume(monkeypatch):
    setup_dummy(monkeypatch)
    prev = json.dumps({"rated_pairs": [{"question": "Q1", "answer": "A1", "rating": 8}]})
...
def test_filter_rated_pairs_and_threshold():
    pairs = [QAPair(question="q", answer="a", rating=5), QAPair(question="q2", answer="a2", rating=9)]
...
def test_apply_curation_threshold(monkeypatch):
    pairs = [QAPair(question="q", answer="a", rating=5), QAPair(question="q", answer="a", rating=7)]
    res_dict = {
        "summary": "s",
...
def test_apply_curation_threshold_dataclass(monkeypatch):
    ...
    res = CurationResult(summary="s", qa_pairs=[], conversations=[], metrics=CurationMetrics(total=1, filtered=0, retention_rate=0.0, avg_score=9), rated_pairs=pairs)
