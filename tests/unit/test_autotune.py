...
def test_recall_at_k_basic():
    ...
    graph = nx.Graph()
    graph.add_node('q', embedding=[1.0, 0.0], graphwave_embedding=[0.5, 0.5], poincare_embedding=[0.2, 0.2])
    graph.add_node('a', embedding=[1.0, 0.0], graphwave_embedding=[0.5, 0.5], poincare_embedding=[0.2, 0.2])
    graph.add_node('b', embedding=[0.0, 1.0], graphwave_embedding=[0.5, -0.5], poincare_embedding=[-0.2, -0.2])
