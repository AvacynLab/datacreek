...
class DummySession:
    ...
    def run(self, query, **params):
        ...
        if "nodeSimilarity.stream('kg_qc')" in query or "nodeSimilarity.stream('kg_sim')" in query:
...
def test_gds_quality_and_node_similarity(monkeypatch):
    ...
    kg.index.build()
    driver = DummyDriver()
    monkeypatch.setattr("datacreek.core.knowledge_graph.get_cleanup_cfg", lambda: {
        "k_min":1, "sigma":0.5, "tau":1, "lp_sigma":0.0, "lp_topk":1, "hub_deg":1
    })
