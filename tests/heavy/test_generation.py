...
def test_bias_reweighting_basic(monkeypatch):
    ...
    scipy_stub = types.SimpleNamespace(
        stats=types.SimpleNamespace(
            wasserstein_distance=lambda a, b: 1.0
        )
    )
...
def test_sheaf_consistency_and_bias(monkeypatch):
    ...
    g = nx.path_graph(2)
    monkeypatch.setattr('datacreek.analysis.sheaf.sheaf_laplacian',
    ...
    monkeypatch.setitem(sys.modules, 'geomloss', types.SimpleNamespace(SamplesLoss=lambda *a, **k: (lambda x,y: 0.2)))
    monkeypatch.setitem(sys.modules, 'torch',
                        types.SimpleNamespace(as_tensor=lambda x, dtype=None: np.asarray(x, dtype=float),
                                             float32=np.float32))
...
def test_format_helpers(monkeypatch):
    monkeypatch.setattr(gen, 'bias_wasserstein', lambda a,b,c: (np.array(c)*0.5, 0.1))
