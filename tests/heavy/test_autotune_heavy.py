...
from datacreek.analysis.autotune import AutoTuneState, recall_at_k, kw_gradient, update_theta

@pytest.mark.heavy
def test_recall_at_k_and_update_theta(monkeypatch):
    g = nx.Graph()
    g.add_node('q', embedding=[1,0], graphwave_embedding=[0,1], poincare_embedding=[1,1])
    g.add_node('a', embedding=[1,0], graphwave_embedding=[0,1], poincare_embedding=[1,1])
    g.add_node('b', embedding=[0,1], graphwave_embedding=[1,0], poincare_embedding=[1,1])
    ...
    state = AutoTuneState()
    metrics = {'cost': 1.2, 'tau':2, 'eps':0.1, 'beta':0.2, 'delta':0.3, 'jitter':0.4}
...
def test_kw_gradient_deterministic(monkeypatch):
    ...
    monkeypatch.setattr(np.random, 'default_rng', lambda: types.SimpleNamespace(choice=lambda _: next(seq)))
