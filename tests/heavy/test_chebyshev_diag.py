...
def test_chebyshev_diag_fallback(monkeypatch):
    ...
    L = np.eye(2)
    res = cd.chebyshev_diag_hutchpp(L, 1.0, order=3, samples=4, rng=np.random.default_rng(0))
    assert res.shape == (2,)
...
def test_chebyshev_diag_scipy_stub(monkeypatch):
    ...
    L = np.diag([1.0, 2.0])
    res = cd.chebyshev_diag_hutchpp(L, 0.1, order=3, samples=4, rng=np.random.default_rng(0))
    assert res.shape == (2,)
