...
def test_ann_latency_histogram(monkeypatch):
    ...
    monkeypatch.setattr('datacreek.core.knowledge_graph.ann_latency', hist, raising=False)
    kg.build_faiss_index()
    kg.search_faiss([1.0, 0.0], k=1)
    buckets = {float(s.labels['le']): s.value for s in hist.collect()[0].samples if '_bucket' in s.name}
