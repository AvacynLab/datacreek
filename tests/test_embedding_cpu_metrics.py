...
def reset_metrics() -> None:
    ...
    embedding_cpu_seconds_per_call._metrics.clear()  # type: ignore[attr-defined]
...
def test_track_embedding_cpu_seconds_increments_counter() -> None:
    ...
    sum_value = REGISTRY.get_sample_value(
        "embedding_cpu_seconds_per_call_sum", {"tenant": "alice"}
    )
    assert total_value is not None and total_value > 0
    assert sum_value == total_value
...
def test_context_manager_updates_cost_when_tracker_provided(monkeypatch) -> None:
    ...
    times = iter([1.0, 3.0])  # 2 seconds of CPU time
    monkeypatch.setattr(time, "process_time", lambda: next(times))
